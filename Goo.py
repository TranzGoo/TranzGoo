...
def folder():
	...
	try:
		ua_ = 'Mozilla/5.0 (iPhone; CPU iPhone OS 10_3 like Mac OS X)'
...
def masuk():
    ...
    rom = raw_input('\n%s [?] Menu : %s'%(P,K))
    if rom in(""):
    	print("%s [!] Ketik yang Benar Gblok "%(M));exit()
    elif rom in ('1','01'):
        jalan("\n%s [%s!%s] Harus Pakai Akun Utama"%(P,M,P))
    	romz = raw_input('%s [?] Token : %s'%(P,K))
        if romz in(""):
        	print("%s [!] Kamu sangat GOBLOK "%(M));exit()
    	try:
            gas = requests.get('https://graph.facebook.com/me?access_token=%s'%(romz)).json()['name']
            print ('\n%s[√] Login berhasil, Mohon Untuk Coli Dulu '%(H));jeda(2)
        	...
        	print("%s [!] Selamat Anda Di Banned "%(M));masuk()
    elif rom in ('2', '02'):
    	...
    	print("%s [!] Isi yang benar ganteng "%(M));exit()
...
class ngentod:
    ...
    def romiy(self):
        ...
        if unikers in ('Y', 'y'):
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '%s [%s!%s] setiap crack 1k ID, gunakan mode pesawat 2 Jam\n'%(P,M,P);jeda(0.2)
            ...
            print '\n%s [ pilih methode crack - silahkan coba satu² ]\n'%(P)
            print ' [%s01%s] methode b-api (ANDALAN#FF200F)'%(K,P)
            print ' [%s02%s] methode mbasic (crack lambat)'%(K,P)
            print ' [%s03%s] methode mobile (crack sangat lambat) '%(K,P)
            self.langsung()
        else:
